<template>
  <ProductContainer :bg="type.bg" class="pt-10">
    <div class="space-y-2">
      ...
      <div
        v-if="contentReady"
        :data-screenshot-content="screenshotName"
        class="pt-2 pb-6 px-2 space-y-2"
      >
        <div class="flex items-center justify-between font-splatoon2">
          ...
          <div v-if="type.badge" class="flex items-center">
            ...
            <div class="font-splatoon2 text-2xl bg-splatoon-blue px-2 drop-shadow rounded">
              {{ $t(type.badge) }}
            </div>
          </div>
        </div>
...
const contentReady = computed(() => hasBattleScheduleContent(store.value.activeSchedule))
</script>
