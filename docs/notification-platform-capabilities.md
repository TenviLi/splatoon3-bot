...
- Keep the bubble below 30 KB and the whole HTTP body below LINE's 2 MB common request limit. A 16:9 screenshot cannot use the full `1024×1024` square without cropping or letterboxing; `1024×576` is the largest exact 16:9 rectangle inside that hard limit. The provider-neutral S3 publisher therefore creates a dedicated LINE PNG under `line-images/`, keeps the lossless result when it is at most 1 MB, and uses a 256-color palette PNG fallback only when needed to meet LINE's recommendation. This variant is independent from `whatsapp-images/` and the primary `BOT_SCREENSHOT_RESOLUTION` image. Before sending, inspect the public object and reject a missing or mismatched image MIME type, unsupported bytes, dimensions above `1024×1024`, or a file above 10 MB.
