...
<table>
  ...
  <tr>
    ...
    <td width="33%" align="center"><strong>Manifest integrity</strong><br><sub>Dimensions, byte counts, hashes, URLs, render time, and time zone remain bound across every stage.</sub></td>
...
```sh
git clone https://github.com/TenviLi/splatoon3-bot.git
cd splatoon3-bot
corepack enable
...
| `pnpm run verify:actions` | Execute `Verify` plus a fixture-backed S3/WeCom Bot Run in OrbStack through `act`. |

## Automation

<p align="center">
  <strong>Data Snapshot</strong> → <strong>Build</strong> → <strong>Screenshot Artifacts</strong> → <strong>S3 Publication</strong> → <strong>Publication Manifest</strong> → <strong>Channel Adapters</strong>
</p>
...
2. **Publish** downloads that archive once, validates each PNG, creates notification variants, publishes both variants through S3, discovers configured `BOT_*_CONFIG` Secrets, and runs all enabled Channel adapters in parallel from the same Node.js process.

Official GitHub Actions are pinned to immutable commit SHAs. [Verify](https://github.com/TenviLi/splatoon3-bot/actions/workflows/ci.yml) runs `pnpm run verify` for pushes to `main`, pull requests, and manual verification runs.
...
Repository Variables expose non-sensitive operational choices without duplicating workflow files:

| Repository Variable | Default | Description |
| --- | --- | --- |
| `BOT_BRANDING_CONFIG` | Required | Strict YAML containing the three public HTTPS branding icon URLs. |
| `BOT_TIME_ZONE` | `Asia/Shanghai` | IANA time zone shared by screenshot rendering and notification formatting. |
| `BOT_RUNNER` | `ubuntu-24.04` | Linux runner label used by both Bot Run stages. Custom runners must provide Chrome runtime libraries. |
| `BOT_ENVIRONMENT` | `production` | GitHub Environment gating the publication job and its deployment policy. |
| `BOT_CONCURRENCY_GROUP` | `splatoon3-bot-production` | Concurrency group serializing publication Bot Runs. |
| `BOT_ARTIFACT_RETENTION_DAYS` | `7` | Screenshot archive retention accepted by `actions/upload-artifact`, from 1 through 90. |
...
- Screenshot capture waits for application readiness, loaded fonts and images, exact viewport geometry, footer position, and zero overflow before writing an artifact atomically.
- Publication verifies the Run Profile, ordered artifact set, stable filenames, exact geometry, byte counts, and SHA-256 digests before any S3 request. It uploads only selected, content-addressed optimized and original PNG variants, then atomically writes a credential-free Publication Manifest bound to the Run Manifest version, Data Snapshot Manifest hash, original artifacts, render time, and time zone.
- The Run Manifest records the validated `BOT_TIME_ZONE`; notification formatting reuses that exact value instead of depending on runner host time.
...
- The notification stage writes a per-Channel summary and fails the Bot Run only after every configured Channel completes.
